...
export const fetchEventsOfChannel = (
	...
	const mahjongChannelEvents: Map<string, NostrEvent | undefined> = new Map<
		string,
	...
	const _subscriptionB = rxNostr.use(rxReqB).pipe(uniq(flushes$)).subscribe({ next, complete });
	rxReqB.emit([
		{
			kinds: [40],
...
export const fetchEventsToReplay = (
	...
	const next = (packet: EventPacket) => {
		...
		if (event.kind === 0) {
			...
			event.tags.some((tag) => tag.length >= 2 && tag[0] === 't' && tag[1] === chatHashtag)
		) {
			...
			.subscribe((packet) => {
				...
				if (event.tags.some((tag) => tag.length >= 2 && tag[0] === 't' && tag[1] === chatHashtag)) {
					...
					if (!chatMembers.has(event.pubkey)) {
						...
						const _subscriptionB3 = rxNostr.use(rxReqB3).pipe(uniq(flushes$)).subscribe({ next });
						rxReqB3.emit({
							kinds: [0],
...
export const fetchProfiles = (rxNostr: RxNostr, players: Map<string, NostrEvent | undefined>) => {
	const next = (packet: EventPacket) => {
		const event = packet.event;
	...
	const flushes$ = new Subject<void>();
	const rxReqB = createRxBackwardReq();
	const _subscriptionB = rxNostr.use(rxReqB).pipe(uniq(flushes$)).subscribe(next);
	rxReqB.emit({
		kinds: [0],
...
export const zap = (pubkey: string, relays: string[]) => {
	if (window.nostrZap === undefined) {
		console.warn('window.nostrZap is undefined');
		return;
	}
...
export const awayuki_mahjong_emojis: { [shortcode: string]: string } = {
